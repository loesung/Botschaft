var initSequence = [
    ...
    'CREATE TABLE message(id TEXT, tag TEXT, data TEXT, timestamp INTEGER)',
    'CREATE TABLE credential(id TEXT, data TEXT)',
    'CREATE TABLE service(id TEXT, identity TEXT)',
];
...
function table($, _, tablename, sqldb){
    ...
    this.insert = function(sets, callback){
        ...
        sqldb.run(sql, function(err){
            if($.types.isFunction(callback)) callback(err, sets);
        });
    };
